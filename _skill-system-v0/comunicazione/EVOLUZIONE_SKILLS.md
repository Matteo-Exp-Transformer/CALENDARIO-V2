...
- 23-08-26 · [metodo] · **UN PERIMETRO DI MANDATO TROPPO STRETTO NON PROTEGGE: PRODUCE DUPLICAZIONE.** Il mandato consentiva di scrivere **solo** in un file; la regula che serviva **esisteva già** altrove ma non era esportata: l'esecutore, rispettando il perimetro, l'ha **replicata**, dichiarando onestamente la duplicazione. **Regola operativa:** prima di scrivere un mandato con perimetro per-file, **cerca la regola che l'attrezzo dovrà applicare**; se vive in un file vietato, o la esporti prima o allarghi il perimetro — mai lasciare che l'agente la copi. Due implementazioni della stessa regola sono un difetto peggiore di quello che la regola governa.
- 23-08-26 · [attenzione] · **CONFERMA lacuna §11 già segnalata — stavolta è costata un blocco vero.** Il mandato citava «§11 di CHIUSURA_SESSIONE.md» **per riferimento**; l'agente ha scritto sei domande **proprie** invece delle sei canoniche `Q1`-`Q6`, e l'hook `Stop` ha giustamente bloccato. **Fix del metodo:** un mandato che chiede la sezione di chiusura **incolla il blocco delle sei domande**, non ci punta. Un formato contabile dalla macchina non si cita: si copia.
- 23-08-26 · [tecnica] · **UN SUB-AGENTE NON PUÒ CREARE FILE NUOVI IN PATH MAI APPROVATI — ed è harness del runtime, non una regola dello skill system.** Il sub-agente ha prodotto capsula e testo ma **non è riuscito a salvare il report** in `docs/Sessioni di lavoro/<data>/`. I sub-agenti non ricevono il prompt di permesso interattivo, quindi la scrittura fallisce. **Comportamento corretto:** registrare il control come **`non_noto` 0/1** invece di dichiarare `pass`. **Regola generale:** se un mandato destinato a un sub-agente prevede la creazione di un file, il path va **pre-autorizzato** negli `allow` prima di lanciarlo.
