 ...
 * conta SOLO le prenotazioni accettate, NON no-show, con orario confermato. Decisione Matteo 11-06-26:
 * i no-show liberano il posto, quindi non occupano coperti.
 */
export function sumGuestsByDate(bookings: BookingRequest[]): Record<string, number> {
  const acc: Record<string, number> = {}
  for (const b of bookings) {
    if (b.status !== 'accepted' || b.no_show || !b.confirmed_start) continue
