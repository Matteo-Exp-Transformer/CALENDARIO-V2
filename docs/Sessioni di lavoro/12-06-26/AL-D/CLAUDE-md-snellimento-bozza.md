# Bozza WP-D4 — testo proposto per `.claude/CLAUDE.md`

> **Stato:** in attesa ok Matteo (file-per-file). Non sostituisce il vivo finché non approvato.
> **Branch verificato:** `env/test`

---

## Testo completo proposto
...
## Note sulla bozza (non fanno parte del file vivo)

1. Rimossi dalla tabella file critici: `vitest.config.ts`, `playwright.config.ts`, `tests/setup.ts`, `.husky/pre-commit`, `.github/workflows/ci.yml`, `validate-invite`, `email.ts` come riga separata (resta in zone delicate), `devConsole.ts` (spostato in § Dev console).
2. Rimossi comandi seed e `supabase db push` / `migration list` — workflow migrazioni in `DB_SKILL.md` / APP_CONTEXT §1b.
