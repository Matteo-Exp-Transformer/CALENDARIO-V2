...
✅ R4 — Al momento del report: sub-agent Prenota **rimandato**. **Aggiornamento controllo 7 report** (batch hook; controverifica didattico è report meta separato): sub-agent poi **PASS** → area **✅ blindata** in `PROSEGUIMENTO_MAPPATURA_SKILL.md`; `courses_label` chiuso in `a79a5af`; cap ingredienti resta **FU-030**. Generalizzazione altre aree (Menu QR). Sistema didattico: solo piano+prompt (intervista profilo → **FU-033**). Propagazione template v.0 (sospesa).
