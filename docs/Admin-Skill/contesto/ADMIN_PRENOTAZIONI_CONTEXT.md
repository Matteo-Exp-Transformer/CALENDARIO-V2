...
| D5, D6, D7, U1, U9, U10 | BASSO | Metadata restore, guard DB assenti, doppio toast, errori UX | ✅ D5 azzera `cancellation_*`; ✅ U1 toast unico; ✅ U10 `logger`; ✅ **U9** (12-06-26): banner `role="alert"` inline nel drawer; ✅ **D6** (Ciclo 4 13-06-26): guard di stato pre-mutation su update/restore/no-show/cancel (`.eq/.neq('status')` + 0-righe → «non più disponibile» + invalidate, pattern accept/reject); ✅ **D7** (Ciclo 4): orario mancante → toast esplicito in `handleSave`, niente salvataggio silenzioso |
| L4, L10–L12 | FU | Ospiti 0/negativi/enormi passano hook — validazione DB/form da valutare | ✅ **Ciclo 4 (13-06-26): cap a video** (decisione Matteo, no server) — clamp input `AdminBookingForm`/`DetailsTab` + messaggio `handleSave` con `BOOKING_PUBLIC_CLIENT_TEXT_LIMITS.numGuestsMax` (110) |
