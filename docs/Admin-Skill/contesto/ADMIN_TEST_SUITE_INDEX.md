...
| `settings-time-slots` | `settingsTimeSlots.settingsM4.adminBlindatura.test.tsx` + `bookingTimeSlots.settingsM4.adminBlindatura.test.ts` + `restaurantSettingRegistry.slotGuestCapacities.settingsM4.adminBlindatura.test.ts` | 20 — enable/disable, add, delete modale in-app (Annulla/Conferma, no `window.confirm`), overlap blocca save, overnight hint, cap per-fascia vs `daily_guest_limit`, cap invalido/alto, delete+save→`deleteServiceSlot`, mutation fail+retry |
