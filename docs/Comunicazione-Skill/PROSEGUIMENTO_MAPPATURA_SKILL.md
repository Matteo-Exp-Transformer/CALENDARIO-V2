...
| **Admin shell + pagine** | 🔶 | **Area 1 Shell:** intervista chiusa + fix flash + `AdminAuthProvider` (debito doppio `useAdminAuth` risolto, autorizzato) + rimosso settings latente sidebar; test `shell-*` verdi. Resta E2E reale per ✅ PROD (debito #1). **Area 2 Prenotazioni: Fase D completata 07-06-26** — conferme coerenti (`BookingDangerActionModal`), 24 test `@admin-blindatura: prenotazioni` (+15 limit test, act() fix), `npm run validate` verde **456 test**. Controtest 4 fronti: **D1 ALTO** (race tab stale accetta→rifiuta sovrascrive accepted), **R1 ALTO** (375px bottoni fuori viewport con textarea), +14 finding MEDIO/BASSO. **Non ✅ PROD** finché Matteo decide fix su D1/R1 (e opz. U2/U6). Report Fase D: questa sessione. |
