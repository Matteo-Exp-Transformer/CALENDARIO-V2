...
| **Admin shell + pagine** | 🔶 | **Area 1 Shell:** intervista chiusa + fix flash + `AdminAuthProvider` (debito doppio `useAdminAuth` risolto, autorizzato) + rimosso settings latente sidebar; test `shell-*` verdi. Resta E2E reale per ✅ PROD (debito #1). **Area 2 Prenotazioni: intervista chiusa 06-06-26 + lavoro prodotto fatto 07-06-26** — `BookingDangerActionModal` unifica le conferme (Elimina/No-show/Reinserisci/Riporta/Rifiuta), tolto `window.confirm()` nativo archivio; test `@admin-blindatura: prenotazioni` (13, verdi); **revisione senior 07-06-26: validate verde 441 test, LOCK booking integri, refactor auth autorizzato**. **Manca solo Fase D** (controtest "ROMPI" + limit test) per chiudere Area 2 a ✅. Report: `docs/Sessioni di lavoro/07-06-26/Report-blindatura-admin-area2-conferme.md`. |
...
**Ordine consigliato:** Prenota ✅ + Menu QR ✅. **Prossimo passo Admin:** chiudere **Area 2 con la
Fase D** (controtest a mandato "ROMPI" + limit test — prompt orchestratore pronto, vedi report
07-06-26), poi Area 3 Settings. Debiti Area 1 da chiudere prima del ✅ PROD finale: E2E reali shell
(#1). Una sola area alla volta. File leggeri e verificati battono tanti file fatti in fretta.
