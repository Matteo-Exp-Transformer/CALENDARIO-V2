...
- 23-08-26 · [tecnica] · **Un contatore che gira in locale può SOTTO-RIPORTARE rispetto alla CI, e nessuno se ne accorge finché la CI non è rossa — misurato: 17 contro 26.** `npm run validate:docs` esce 1 con **17 path rotti** sulla macchina di Matteo e con **26** nel clone pulito di GitHub Actions (run `32652259771`). Non è un bug dello script: i 9 di differenza sono link verso `docs/_lavoro/` (privato) e `.cursor/plans|hooks/*` (gitignored) — **esistono sul disco, non in git**, quindi in locale il controllo li «trova». Il numero 17 era stato citato come *il* numero in almeno quattro report del 22-08 e del 23-08: un dato **vero e fuorviante insieme**, il tipo peggiore. **Regola generalizzabile a qualunque controllo che risolve path o legge file:** un controllo che tocca il filesystem va misurato **almeno una volta in un clone pulito** prima di scrivere il suo numero in un documento, e il numero va scritto con l'ambiente accanto («17 in locale / 26 in CI»), mai da solo. Controprova meccanica a costo zero: `git archive HEAD | tar -x -C <tmp>` e rilancio lì. ⚠️ **Corollario sul rimedio:** per i 9 casi la correzione del link è **impossibile per costruzione** (il bersaglio non deve entrare in git), quindi la strada giusta è la deroga dichiarata, non la riparazione — distinguere «link rotto» da «link a materiale legittimamente assente» è parte del criterio, non un'eccezione ad esso. **NON propagato a `_skill-system-v0/`:** prima occorrenza. **Candidato Playbook** se si ripresenta su un altro controllo (lint, test, generatore).
- 23-08-26 · [metodo] · **Le regole si INCOLLANO nel mandato, non si citano — ora con la prova positiva accanto a quella negativa.** Il 13-06-26 era stata segnalata come lacuna; il 23-08-26 è costata un blocco vero (un mandato *citava* `CHIUSURA_SESSIONE.md` §11 invece di incollare il blocco delle sei domande, e l'agente ne ha scritte sei **sue**: hook rifiutato, un giro perso). **La prova positiva è arrivata lo stesso giorno:** i mandati per Codex e Cursor contenevano una tabella «trappole già pagate» con le regole **scritte per esteso** (`npm.cmd` su Windows, UUIDv7 e non v4, `segment_no` sempre `1`, il doppio giro dell'hook di pre-commit) — e nei report delle fasi B/C/D/E **risultano tutte rispettate**, zero occorrenze delle trappole citate. Due occorrenze, segno opposto, stesso giorno. **Regola:** un mandato che rimanda a un file per una regola che l'agente deve applicare **alla lettera** ha già fallito; il puntatore va bene per il *contesto da leggere*, mai per il *formato da produrre*. Costo dell'incollatura: ~25 righe per mandato. Costo del puntatore: un ciclo di lavoro. **Proposta operativa:** `PREPARA_PROMPT_SKILL.md` deve obbligare a incollare il blocco Q1–Q6 in ogni mandato che chiede un report — oggi non lo fa (verificato: zero occorrenze di «perimetro», `validate:mss`, `D17`, `D18` in quel file). Confluisce nel mandato di revisione `Prompt-revisione-skill-chiusura-e-hook-23-08-26.md`. **NON propagato a `_skill-system-v0/`:** il template non contiene MSS (verificato: `find` su `*mss*`/`*meta*` a vuoto); la sede naturale è `PREPARA_PROMPT_SKILL.md`. **Candidato Playbook:** seconda occorrenza accertata — soglia ≥2 raggiunta, da promuovere.
...
- 23-08-26 · [metodo] · **UN PERIMETRO DI MANDATO TROPPO STRETTO NON PROTEGGE: PRODUCE DUPLICAZIONE.** Seduta MSS «vista effettiva» (esecutore sub-agente Sonnet, revisione Opus, cantiere `SK-6`). Il mandato consentiva di scrivere **solo** in `scripts/mss/query.mjs`. La logica che serviva — applicare la catena degli `amendment` — **esisteva già** in `core.mjs::applyAmendmentsView()`, ma non era esportata: l'esecutore, rispettando il perimetro, l'ha **replicata** (~120 righe), dichiarando onestamente la duplicazione in output. Matteo: *«dobbiamo snellire, non duplicare»* → perimetro allargato, funzione esportata, `query.mjs` ridotto a un adattatore che delega. Risultato identico (13 campi applicati, 0 non risolte), una sola implementazione. **Regola operativa (ora `D18` in `PLAN_V0` §15):** prima di scrivere un mandato con perimetro per-file, **cerca la regola che l'attrezzo dovrà applicare**; se vive in un file vietato, o la esporti prima o allarghi il perimetro — mai lasciare che l'agente la copi. Due implementazioni della stessa regola sono un difetto peggiore di quello che la regola governa. **Corollario per i revisori:** quando un output dichiara «allineato a X, non importato da X», quello **non** è un limite accettabile, è debito da chiudere subito.
- 23-08-26 · [attenzione] · **CONFERMA della lacuna §11 già segnalata il 13-06-26 — stavolta è costata un blocco vero.** Il mandato citava «§11 di CHIUSURA_SESSIONE.md» **per riferimento**; l'agente ha scritto sei domande di chiusura **proprie** invece delle sei canoniche `Q1`-`Q6`, e l'hook `Stop` ha giustamente bloccato. **Fix del metodo:** un mandato che chiede la sezione di chiusura **incolla il blocco delle sei domande**, non ci punta. Un formato contabile dalla macchina non si cita: si copia. La proposta del 13-06 (nota in CLAUDE.md alla voce «lavoro ok») resta valida e ora ha due occorrenze a supporto.
- 23-08-26 · [tecnica] · **UN SUB-AGENTE NON PUÒ CREARE FILE NUOVI IN PATH MAI APPROVATI — ed è harness Anthropic, non una regola dello skill system.** Il sub-agente esecutore ha prodotto capsula e testo ma **non è riuscito a salvare il report** in `docs/Sessioni di lavoro/<data>/`. Verificato: `.claude/settings.local.json` non ha alcun `deny` né hook su `Write`/`Edit` — i sub-agenti semplicemente non ricevono il prompt di permesso interattivo, quindi la scrittura fallisce. **Comportamento corretto tenuto dall'agente, da imitare:** invece di dichiarare `pass` sulla prova che non poteva eseguire, ha registrato il control come **`non_noto` 0/1**. **Fix applicato:** aggiunti agli `allow` `Write/Edit(docs/Sessioni di lavoro/**)` e `Write/Edit(docs/MetaSkillSystem/**)`. **Regola generale:** se un mandato destinato a un sub-agente prevede la creazione di un file, il path va **pre-autorizzato** negli `allow` prima di lanciarlo, altrimenti il lavoro arriva completo ma non salvato.
