...
- 18-06-26 · [tecnica] · **MCP `deploy_edge_function` + `_shared/` cross-function** — il bundler MCP mette i file in `source/` e l'import `../_shared/log.ts` cerca il file un livello sopra (`_shared/log.ts` nella root tmp). Per far trovare il file: nominarlo `../_shared/log.ts` nell'array `files` della chiamata MCP. Documentare in `DB_SKILL.md` §Deploy il path trick (ora solo in Q5 report 18-06-26).
