...
- **Stato:** 🔶 parziale 12-06-26 — chiusi auth admin rimossa, popup nativi, **FU-TYPES-1** (hook Supabase
  perimetro typed), fallback demo `booking_placement_areas`, **orari default, sfondo e form config Pagina
  Prenota** (EmptyState se `booking_public_form_config` assente), guard dirty su CRM + overlay Categorie
  Menu + collapse nuova prenotazione, `menuQrStorage` senza `as any`.
  Restano email (FU-EMAIL), logging edge/scripts (FU-LOG-1), guard Servizio/modali Pro (FU-023),
  Admin Area 2 residui UX (FU-046 U3/U9…), M4/M5.
...
| FU-EMAIL-1 | ~~Edge function `send-email` non esiste~~ — **Chiuso 15-06/17-06-26; aggiornato 19-06-26:** `send-email` deployata in PROD (`rwuxgvld` **v6**) + secret Brevo; accetta/rifiuta ricevuti. Tabelle `email_templates`/`email_campaigns` allineate su PROD (mig. 050-052, 17-06). Gate client `VITE_ENABLE_SEND_EMAIL`. Opt-out marketing per-cliente chiuso da FU-EMAIL-12: mig. 055 `unsubscribe_tokens` + Edge `unsubscribe` v1. Resta FU-EMAIL-8 (scheduler campagne). | `src/lib/email.ts`; `useBookingMutations.ts`; `emailTemplates.ts` | — | chiuso |
