...
- **Salva sottotab:** unico punto `saveModesSection` (footer **Salva modifiche**): integra bozze (`draftSubTabsByMode`), valida, poi `persistModesSection` (chiude editor al successo).
- **Dettaglio promo (CRUD):** persistenza con footer; delete promo con modale in-app (non
  `window.confirm`); conflitto abbinamento → `PromoPlacementConflictDialog`. Vedi report promo 29-05-26.
- **Editor sottotab Card scorrevole** (`display === 'cards'`): titolo tecnico nell'editor aperto `Card N` / bozza `Nuova card · Card N` (o `Titolo · Card N` se già digitato); campo **Titolo card** (24) per il testo sulla Pagina Prenota — **vuoto** su nuova card (`newSubTab` → `label: ''`), placeholder **«Nome card scorrevole»**; campo **Badge prenotazione** (`sub_tabs[].booking_badge_label`, max **12**) solo per la card prenotazione admin, con switch `sub_tabs[].booking_badge_enabled` (omesso/true = visibile, `false` = nascosto) e fallback al Titolo card. **Titolo card** è riempito col nome del menù solo dopo scelta in **Importa menù preselezionato**; tornando a **Compila manualmente** (`preset_id` cleared) anche `label` si azzera. Select con tutti i `booking_custom_staff_presets`, senza filtro tipologia/visibilità. Descrizione breve (**79**); **Numero Portate** opzionale (`courses_label`, max **12**); Icona; «Categorie e ingredienti visibili» solo con `preset_id`; toggle **Menù personalizzabile** solo con `preset_id`; sotto il toggle, se `is_fixed_menu: false`: lista checkbox per-categoria che individua `compilable_category_keys?: string[]` — le chiavi selezionate mostrano checkbox al cliente nella Pagina Prenota; le altre compaiono in sola lettura (FIX 9 §3A admin + §3B pubblica). Campo assente = tutte le categorie compilabili (backward compat). Prezzo (live preset se non personalizzato). `parseSubTabFromUnknown` scarta card senza `label` compilata.
- **Editor sottotab carosello** (`display === 'carousel'` + `BookingFormCarouselEditor`): flusso **foto-first**; campi per slide: **Testo Etichetta** → `eyebrow`, **Testo Titolo** → `title`, **Scegli Icona** → `icon`, **Testo Etichetta / Titolo / Descrizione** → `eyebrow` / `title` / `description` (max **19 / 18 / 38** — `BOOKING_CAROUSEL_SLIDE_TEXT_LIMITS` in `bookingPrenotaTextLimits.ts`). Migrazione dati legacy: `040_clamp_booking_carousel_slide_text_limits.sql` su `restaurant_settings.booking_public_form_config`. Intestazione slide: **Foto N° X** (ordine carosello, 1 = prima a sinistra). Pulsante matita **Modifica foto** (`replaceAt` su `useCarouselPhotoUpload`) accanto a rimuovi. **Blocco prezzo** (solo label + input €, opzionale). **Sotto**, blocco toggle stile «Menù personalizzabile»: titolo «Mostra dettaglio offerta» + help `text-xs` (nome carosello + titoli slide nel riepilogo; prezzo indipendente, solo se compilato); switch `show_offer_details_in_summary` a destra. **Non** è in `field_overrides`. Helper pubblici: `getShowOfferDetailsInSummary`, `resolveCarouselSummaryDisplay`, `getCarouselStickyMiniPanelLine`. **Riepilogo** (`BookingSummarySidebar`): con toggle ON (default) mostra `label` carosello in righe «Tipo» e «Opzione menu» + titoli slide in «Offerta selezionata»; con toggle OFF nasconde nome e titoli (righe «Tipo»/«Opzione menu» assenti se non resta altro); prezzo a persona nel blocco carosello solo se `price_per_person` > 0, indipendentemente dal toggle. Sticky bar mobile segue `getCarouselStickyMiniPanelLine`. Nessun toggle fisso/personalizzabile. Upload bucket `menu-photos`, path dedicato Prenota `{tenantId}/booking-form/{modeId}/{subTabId}/carousel/{uuid}.webp` (non `qr/...`).
- **Sottotab salvate (card + carosello)** in lista: riga compatta con titolo da `getSubTabCollapsedRowTitle` — card: **`{label trimmato} · Card N`**; carosello: **Nome carosello** / `Carosello N`. Azioni nella testata; click apre/chiude (`expandedSubTabByMode`). Editor distinti: card = Titolo card + Badge prenotazione (testo + switch visibilità) + import preset + visibilità + toggle + prezzo; carosello = Nome carosello + `BookingFormCarouselEditor`. Titolo pubblico card solo da campo **Titolo card** (24); `booking_badge_label` / `booking_badge_enabled` non cambiano la Pagina Prenota. Carosello senza matita né frecce sposta sulle card. Ordine editor card: campi base → categorie → toggle personalizzabile → prezzo. Dopo **Salva modifiche** footer con persist OK, `persistModesSection` chiude editor e bozze.
- **Centratura elemento aperto:** quando Mario apre una modalità, una card scorrevole salvata o una
  bozza card/carosello, `BookingFormConfigPanel` centra l'elemento appena aperto nel contenitore
  scrollabile admin tramite helper riusabile `src/features/booking/lib/scrollIntoCenter.ts`
  (`prefers-reduced-motion` rispettato; niente scroll se già centrato). Per ora applicato solo a
  Personalizza form; estensione agli altri modal tracciata in `docs/FOLLOW_UP.md` (FU-055).
...
- **XOR card/carosello per modalità** (`sub_tabs_presentation: ‘cards’ | ‘carousel’ | null`):
  ...
  - `SubTabsPresentationBadge` con «Modalità impostata» + link «Cambia presentazione» (richiede conferma + cancella sottotab esistenti).
    La conferma è una `Modal` in-app, non popup nativo del browser.
  - `resetSubTabsPresentation(modeId)` azzera `sub_tabs_presentation` e `sub_tabs[]` senza toccare il DB finché non si Salva.
  - Se l'ultima sottotab salvata viene eliminata, `sub_tabs_presentation` torna `null`: la UI deve
    mostrare di nuovo la scelta «+ Card scorrevole» / «+ Carosello». Stessa cosa se una bozza viene
    annullata e non restano sottotab salvate.
  - Una card scorrevole senza **Titolo card** **non si salva** come sottotab valida. Importare un
    menù preselezionato compila il titolo col nome preset, ma Mario può modificarlo; se lo svuota,
    il salvataggio deve bloccarsi.
  - Un carosello senza almeno una foto **non si salva** come sottotab valida; aggiungere una foto o
    eliminarlo. Il normalizer/parser rimuove eventuali caroselli vuoti già presenti in DB.
  ...
  - **Pagina pubblica**: con `sub_tabs_presentation === 'carousel'` `BookingRequestForm` **non** renderizza `BookingSubTabCards` (la strip selettore) e auto-seleziona l'unica sottotab carosello via `useEffect`; viene mostrato direttamente `BookingSubTabCarousel`. Se non ci sono sottotab valide, **non** mostra la vecchia `MenuSelection` di fallback, anche quando `sub_tabs_enabled=false`.
...
- **Tracking personalizzazioni (`field_overrides`)**: ogni patch ai campi vetrina della sottotab (`label`, `description`, `price_per_person`, `hidden_item_ids`, `hidden_category_keys`, `category_order_keys`) viene automaticamente marcato come «personalizzato» (`true`) da `applyPatchWithOverrideTracking`. **Importa menù preselezionato** azzera tutti gli override (`presetImportFieldOverrides()`), così cambi futuri al preset si propagano live alla pagina Prenota. Resolver in `src/features/booking/services/bookingFormResolver.ts` applicato lato pubblico in `BookingRequestForm.activeModeSubTabs`. Test: `src/features/booking/services/__tests__/bookingFormResolver.test.ts`.
- **Ordine categorie ingredienti (Pagina Prenota)**: in «Categorie e ingredienti visibili» (solo card con `preset_id`) frecce su/giù riordinano `sub_tabs[].category_order_keys` per quella sub-tab; override in `field_overrides.category_order_keys`. Fallback assente → `menu_categories.sort_order` (come prima). Non tocca Menu QR. Utility: `orderCategoryKeys` in `src/features/booking/utils/orderCategoryKeys.ts`. Sync rename/delete categoria: `bookingFormCategoryKeySync.ts` (parallelo a `hidden_category_keys`).
...
- **Badge prenotazione admin:** `booking_modes[].booking_badge_label` e `sub_tabs[].booking_badge_label` sono opzionali, max **12**, e servono solo alla card prenotazione in Admin → Calendario → Giorno. `booking_modes[].booking_badge_enabled` / `sub_tabs[].booking_badge_enabled` spengono il singolo badge quando valgono `false`; omesso o `true` = visibile. Se il testo è vuoto: fallback a `booking_modes[].label` e, solo per card scorrevoli, `sub_tabs[].label`. Il nome carosello non viene mostrato come terzo badge. Non sono `field_overrides`, non cambiano titolo pubblico, riepilogo o submit.
...
- **Categorie ingredienti pubblico:** `BookingMenuCategoryCard` parte chiusa con foto categoria e nome categoria; al click la foto categoria sparisce e viene mostrata la lista ingredienti della categoria. Un nuovo click sull'intestazione categoria richiude la lista e ripristina la foto categoria. Quando cambia card/preset selezionato, le categorie tornano chiuse. **Desktop carosello (≥700px, `ComposeScrollRow`):** una sola categoria aperta alla volta; frecce avanti/indietro o scroll che porta la card fuori dal viewport del carosello chiudono il pannello (dettaglio §7 `PRENOTA_LAYOUT_CONTEXT.md`).
...
| Badge prenotazione admin | **12** (`booking_badge_label`, tipologia + card scorrevole) |
