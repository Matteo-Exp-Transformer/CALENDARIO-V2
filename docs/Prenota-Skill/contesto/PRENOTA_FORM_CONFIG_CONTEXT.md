...
- **Sottotab salvate (card + carosello)** in lista: riga compatta con titolo da `getSubTabCollapsedRowTitle` — card: **`{label trimmato} · Card N`**; carosello: **Nome carosello** / `Carosello N`. Azioni nella testata; click apre/chiude (`expandedSubTabByMode`). Editor distinti: card = Titolo card + Badge prenotazione + import preset + visibilità + toggle + prezzo; carosello = Nome carosello + `BookingFormCarouselEditor`. Titolo pubblico card solo da campo **Titolo card** (24); `booking_badge_label` non cambia la Pagina Prenota. Carosello senza matita né frecce sposta sulle card. Ordine editor card: campi base → categorie → toggle personalizzabile → prezzo. Dopo **Salva modifiche** footer con persist OK, `persistModesSection` chiude editor e bozze.
...
- **Badge prenotazione admin:** `booking_modes[].booking_badge_label` e `sub_tabs[].booking_badge_label` sono opzionali, max **12**, e servono solo alla card prenotazione in Admin → Calendario → Giorno. Se vuoti: fallback a `booking_modes[].label` e, solo per card scorrevoli, `sub_tabs[].label`. Il nome carosello non viene mostrato come terzo badge. Non sono `field_overrides`, non cambiano titolo pubblico, riepilogo o submit.
