...
- **Badge prenotazione admin:** `booking_modes[].booking_badge_label` e `sub_tabs[].booking_badge_label` sono opzionali, max **18**, e servono solo alla card prenotazione in Admin → Calendario → Giorno. Se vuoti: fallback a `booking_modes[].label` e `sub_tabs[].label`. Non sono `field_overrides`, non cambiano titolo pubblico, riepilogo o submit.
...
| Badge prenotazione admin | **18** (`booking_badge_label`, tipologia + card) |
