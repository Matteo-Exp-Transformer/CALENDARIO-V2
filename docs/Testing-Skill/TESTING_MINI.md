...
3. **Profilo Verifica = QA manuale obbligatorio (§8):** `npm run validate` poi stessi casi funzionali
...
- **Viewport standard 375/834/1280** (+400px solo admin Calendario) → `TESTING_SKILL.md` §8.2.
- **`npm run validate`** è il gate pre-PR (lint + typecheck + Vitest) → §3 + §8.1.
