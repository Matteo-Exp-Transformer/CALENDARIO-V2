...
> **Fonte di verità (non questo riepilogo):** elenco file in `supabase/migrations/` + stato remoto via MCP `list_migrations` dopo `get_project_url` (TEST `docnnernvp`, PROD `rwuxgvld` sola lettura). Se MCP TEST non è autorizzato, usare la checklist CLI TEST in `APP_CONTEXT_SKILL.md` §1b. Dettaglio workflow e anomalie storiche: `Database-Skill/DB_MIGRATIONS_CONTEXT.md`.
...
> **Ambiente (agg. 2026-06-12)**: lo sviluppo punta al **server di TEST** (`docnnernvp`). Canale preferito: MCP `Supabase_test`; fallback operativo attuale: CLI per TEST dopo checklist §1b. Produzione (`rwuxgvld`) è sola lettura salvo richiesta esplicita e oggi passa dal MCP PROD. Dettaglio in `APP_CONTEXT_SKILL.md` §1b.
