...
| 049 | `049_menu_qr_item_sort_overrides.sql` | `menu_qr_codes.item_sort_overrides` — ordine piatti per-QR (`{ "category_key": ["item_uuid", …] }`); chiave assente = ordine magazzino (`sort_order` + `name`) |
| 050 | `050_email_templates.sql` | Tabella per-tenant degli override testuali email (accetta/rifiuta/promo). RLS solo admin del proprio tenant + FORCE RLS. Campi nullable → fallback cablato nel codice. **Presente anche in PROD** |
| 051 | `051_email_campaigns.sql` | Tabella per-tenant delle campagne email, **limite duro 5 per tenant** via trigger BEFORE INSERT. RLS solo admin + FORCE RLS. **Presente anche in PROD** |
| 052 | `052_email_campaigns_heading.sql` | Colonna `heading` opzionale sulle campagne; `null` = `DEFAULT_CAMPAIGN_HEADING` del builder. **Presente anche in PROD** |
| 053 | `053_marketing_consent.sql` | Consenso marketing email su `booking_requests` e `customers` (art. 6.1.a GDPR). Default `false`: mai attivo senza spunta esplicita; il rifiuto non blocca la prenotazione |
| 054 | `054_dietary_consent.sql` | `dietary_data_consent` su `booking_requests` (art. 9.2.a GDPR, dati di salute). Default `false`; l'Edge `create-booking` **rifiuta con 400** una prenotazione con dati alimentari e senza consenso |
| 055 | `055_unsubscribe_tokens.sql` | Disiscrizione marketing via link email (FU-EMAIL-12): token opaco per-destinatario emesso da `send-email` (service_role) e consumato dall'Edge pubblica `unsubscribe`. ⚠️ La tabella non deve **mai** essere raggiungibile dal client. **Presente anche in PROD** |
| 056 | `056_booking_admin_notes.sql` | Appunti interni admin sulla singola prenotazione (solo dal modal dettaglio). Colonna additive-only, nessun GRANT/RLS nuovo. Applicata su TEST il 21-06-26 (`20260621015549`); PROD: vedi `FU-056-PROD` |
| 057–058 | durata fasce + snapshot prenotazione | solo TEST; fondamenta S2. `058_booking_requests_duration_snapshot.sql` = snapshot durata congelata (D14), nullable e retrocompatibile (`NULL` = permanenza OFF, D15) |
