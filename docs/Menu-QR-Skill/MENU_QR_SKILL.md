...
QR**, non in un «tipo di menù» strutturato. Vedi §3 (il `content_type` è codice morto).
...
## 3-bis. CODICE MORTO da rimuovere — `content_type` / preset / menù-evento via QR

> **Scoperta della sessione di mappatura (06-06-26), decisa con Matteo.** È il pezzo di «senso» che
> solo la mappatura fa emergere. Un agente NON deve costruirci sopra.

Nel codice un QR ha un campo `content_type` con tre valori (`a_la_carte` / `preset_menus` / `mixed`)
che attiverebbe i **menù-evento (preset)** dentro il QR: un tab «eventi», una pagina dedicata
`PublicMenuPresetPage`, rami `showPresets`. **MA il modale `MenuQrModal` NON ha alcun controllo per
sceglierlo:** salva sempre `content_type: 'a_la_carte'` e non tocca mai `preset_ids`. Quindi:

> **Tutta la logica preset/mixed nel pubblico è codice IRRAGGIUNGIBILE dall'interfaccia.** Nessun QR
> può essere creato come «misto» o «solo-eventi». Per questo Matteo «non li vede e non sa cosa siano»:
> nell'esperienza reale **non esistono**.

**Decisione di Matteo:** il caso «evento» si copre già col **carosello + nome QR** (foto dell'evento,
nome dell'evento); il `content_type` strutturato **non serve → DA RIMUOVERE**. Oggi mappatura soltanto.

**Conseguenza per gli INC «aperti» dei report vecchi:** gli incidenti che riguardano la pagina preset
(INC-05 foto, INC-06 tab eventi nasconde categorie, INC-15 hidden items, INC-16 `tenantReady`) sono
**latenti/irraggiungibili**, non urgenti — vivono su codice che andrà tolto. NON «fixarli»: appartengono
alla rimozione. Cosa togliere quando si farà la pulizia → `contesto/MENU_QR_DATA_FLOW_CONTEXT.md`
(sezione «Codice morto da rimuovere»).
...
- Nome QR: **80** · Carosello: etichetta **40**, titolo **60**, descrizione **125** (con contatore
...
## 5. Questioni aperte (decise, da implementare)

> Non dimenticanze: decisioni di Matteo in attesa di una sessione di esecuzione. Un agente che le
> incontra NON le «sistema» di iniziativa; può proporne l'implementazione se il task è pertinente.

- **FU-MQR-1 — Titoli/descrizioni categoria per-QR SENZA cap → da cappare.** Nel modale, i due campi
  «Titolo card» e «Descrizione breve» per categoria (`MenuQrCategoryCardsSection`) sono `<input>`
  semplici **senza `maxLength` né contatore**, a differenza del carosello. Decisione (06-06-26):
  **vanno cappati** con limiti anti-rottura mobile (stesso principio del §3) e meglio con
  `AdminFieldWithCharCount`. Punto esatto nel codice e numeri proposti:
  `contesto/MENU_QR_TEXT_LIMITS_MAP.md` (sezione «Da cappare»).
- **Rimozione codice morto `content_type`/preset** (vedi §3-bis) — sessione di pulizia dedicata.
...
| Flusso dati admin ↔ pubblico, `category_filter`, `category_images`, `hidden_menu_item_ids`, rename/delete chiave categoria, hook pubblici/admin, **codice morto da rimuovere** | `contesto/MENU_QR_DATA_FLOW_CONTEXT.md` **(OBBLIGATORIO prima di modificare dati)** |
