...
2. **Remoto (sola lettura per consultazione):** MCP `get_project_url` → deve rispondere TEST `docnnernvp` (sviluppo) o PROD `rwuxgvld` (sola lettura salvo conferma esplicita). Poi `list_migrations`. Se MCP TEST non è autorizzato ma CLI sì, usare la checklist CLI TEST di `APP_CONTEXT_SKILL.md` §1b e `npx supabase migration list --linked`.
...
| `048_schedule_rate_limits_cleanup.sql` | WP-B5: abilita `pg_cron`, definisce `cleanup_rate_limits()` e programma job orario `cleanup-rate-limits-hourly`. PROD applicata/verificata 12-06-26 (`20260612131057`); TEST pendente: CLI vede il progetto e il registro, MCP TEST non autorizzato |
...
- **TEST/staging**: `docnnernvpyrbwuzzach.supabase.co` — MCP `user-supabase-test` quando autorizzato; fallback CLI con checklist §1b `APP_CONTEXT_SKILL.md`. Ambiente di sviluppo abituale.
...
### Snapshot remoto TEST (12-06-26, CLI dopo checklist → docnnernvp)

Snapshot precedente: registro remoto includeva tutte le migrazioni fino a `045_menu_magazzino_is_available` (versione timestamp `20260611193908`). Dopo WP-B1/WP-B2 risultano versionate anche `046` e `047` nel repo. Diagnosi 12-06-26: CLI `migration list --linked` su progetto linkato `docnnernvpyrbwuzzach` legge il remoto fino a `20260612111433`; `048` resta non applicata. MCP `get_project_url`/`list_migrations` su TEST fallisce con permission denied perché il connettore vede solo l'organizzazione PROD.
...
# Preferito: MCP Supabase test, get_project_url deve rispondere docnnernvp.
# Fallback TEST: CLI solo dopo checklist APP_CONTEXT §1b e conferma esplicita se scrive SQL.

# 3. Applica via MCP sul DB TEST
# Supabase_test__apply_migration(name, query)
# Se MCP TEST non è disponibile: non usare db push; definire prima strategia db query + registro migrazioni.
