...
- **Key SOLO-ADMIN** (`app_theme`, `booking_placement_areas`, `slot_guest_capacities`, `slot_limit_enabled`, `booking_reject_out_of_slot`, `booking_time_slots_enabled`, `walk_in_max_guests`, `timezone`, `booking_window_days`): NON in whitelist → invisibili ad anon. La dashboard le legge dal client autenticato `supabase` passando `useRestaurantSetting(key, { authenticated: true })` (policy `admin_*` di 002). I limiti pubblici (cap **per-fascia**) restano applicati dalle Edge Function con service_role. ⚠️ *Correzione 05-08-26: l'elenco conteneva `daily_guest_limit`, **rimosso il 18-06-26** col cambio di modello (non c'è più un cap giornaliero). La policy della `047` non è stata toccata: la key semplicemente non esiste più lato app.*
