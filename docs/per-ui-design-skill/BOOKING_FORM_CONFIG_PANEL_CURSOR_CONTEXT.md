...
- **XOR card/carosello per modalità** (`sub_tabs_presentation: ‘cards’ | ‘carousel’ | null`):
  ...
  - `’cards’` o `’carousel’` → `SubTabAddButtons` mostra solo il pulsante coerente; compare `SubTabsPresentationBadge` con «Modalità impostata» + link «Cambia presentazione» (richiede conferma + cancella sottotab esistenti).
