...
- **Editor sottotab carosello** (`display === 'carousel'` + `BookingFormCarouselEditor`): flusso **foto-first**; campi per slide: **Testo Etichetta** → `eyebrow`, **Testo Titolo** → `title`, **Scegli Icona** → `icon`, **Testo Descrizione** → `description` (max 19/18/38 — `BOOKING_CAROUSEL_SLIDE_TEXT_LIMITS` in `bookingPublicFormConfig.ts`). Intestazione slide: **Foto N° X** (ordine carosello, 1 = prima a sinistra). Pulsante matita **Modifica foto** (`replaceAt` su `useCarouselPhotoUpload`) accanto a rimuovi. Prezzo opzionale a livello sottotab: in Prenota viene moltiplicato per il numero ospiti e mostrato nel riepilogo insieme ai titoli delle foto. Nessun toggle fisso/personalizzabile. Upload bucket `menu-photos`.
...
**Prezzo carosello**: non viene mostrato sulle singole slide; se `sub_tabs[].price_per_person` è impostato, il riepilogo mostra prezzo a persona, numero ospiti e totale. Il riepilogo elenca anche i titoli delle foto del carosello.
