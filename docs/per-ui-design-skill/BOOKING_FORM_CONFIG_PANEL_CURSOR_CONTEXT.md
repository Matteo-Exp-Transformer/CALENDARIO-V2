...
- **Editor sottotab Card scorrevole** (`display === 'cards'`): Etichetta card (60), Icona, **Importa menù preselezionato** (select con tutti i `booking_custom_staff_presets`, senza filtro tipologia/visibilità), toggle **Menù fisso**, Prezzo, Descrizione breve (80); sezione «Categorie e ingredienti visibili» solo con `preset_id`. Re-import preset **non** sovrascrive un’etichetta già personalizzata (diversa dal nome del preset precedente).
...
- **Sottotab salvate (card + carosello)** in lista: riga compatta con titolo tecnico (`Card N` / `Carosello N`) e azioni nella testata; click sulla riga apre/chiude (`expandedSubTabByMode`). Le card hanno titolo modificabile inline con matita (blur/Invio salva nel campo `label`); il carosello mostra solo `Carosello N`, senza matita né frecce sposta. Nelle card, ordine editor: campi base → categorie/ingredienti → toggle **Menù personalizzabile** → prezzo; se il toggle e attivo il prezzo e svuotato/disabilitato. Card e carosello hanno il blocco prezzo allineato a sinistra nel form, con simbolo `€` dentro il campo e senza testo descrittivo sotto. **Salva** (`commitSubTabEditor`) chiude il pannello.
...
- Il **Carosello** salva testi/icona per slide in `carousel_items[]` (`eyebrow`, `title`, `description`, `icon`); il titolo tecnico nell'editor non e modificabile e `sub_tabs[].label` non viene sovrascritto dai testi slide; può salvare `price_per_person`, ma non `is_fixed_menu` né `description` a livello sottotab carosello.
