...
- **Editor sottotab carosello** (`display === 'carousel'` + `BookingFormCarouselEditor`): flusso **foto-first**; campi per slide: **Testo Etichetta** → `eyebrow`, **Testo Titolo** → `title`, **Scegli Icona** → `icon`, **Testo Descrizione** → `description` (max 30/22/66). Intestazione slide: **Foto N° X** (ordine carosello, 1 = prima a sinistra). Pulsante matita **Modifica foto** (`replaceAt` su `useCarouselPhotoUpload`) accanto a rimuovi. Prezzo opzionale a livello sottotab (nota: in Prenota `price_per_person` viene azzerato dal resolver per `display='carousel'`); nessun toggle fisso/personalizzabile. Upload bucket `menu-photos`.
...
**Nessun prezzo** su slide e nel riepilogo: per `display='carousel'` `price_per_person` viene azzerato dal resolver (quindi non viene mostrato in Prenota).
