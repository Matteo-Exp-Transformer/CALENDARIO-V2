...
- **Editor sottotab Card scorrevole** (`display === 'cards'`): titolo tecnico `Card N` solo per orientare l'admin; campo **Titolo card** (30) separato per il testo mostrato in pagina Prenota, precompilato dal nome del menù importato e sovrascrivibile; Icona; **Importa menù preselezionato** (select con tutti i `booking_custom_staff_presets`, senza filtro tipologia/visibilità); Descrizione breve (80); sezione «Categorie e ingredienti visibili» solo con `preset_id`; toggle **Menù personalizzabile**; Prezzo. Re-import preset **non** sovrascrive un’etichetta già personalizzata (diversa dal nome del preset precedente).
- **Editor sottotab carosello** (`display === 'carousel'` + `BookingFormCarouselEditor`): flusso **foto-first**; campi per slide: **Testo Etichetta** → `eyebrow`, **Testo Titolo** → `title`, **Scegli Icona** → `icon`, **Testo Descrizione** → `description` (max 30/22/66). Intestazione slide: **Foto N° X** (ordine carosello, 1 = prima a sinistra). Pulsante matita **Modifica foto** (`replaceAt` su `useCarouselPhotoUpload`) accanto a rimuovi. Prezzo opzionale a livello sottotab; nessun toggle fisso/personalizzabile. Upload bucket `menu-photos`.
- **Sottotab salvate (card + carosello)** in lista: riga compatta con titolo tecnico (`Card N` / `Carosello N`) e azioni nella testata; click sulla riga apre/chiude (`expandedSubTabByMode`). I due editor restano distinti: card usa campi titolo/import preset/visibilita/toggle/prezzo; carosello usa solo prezzo + `BookingFormCarouselEditor` per slide. Le card modificano il titolo pubblico solo tramite campo **Titolo card** (30), non dalla testata tecnica; il carosello mostra solo `Carosello N`, senza matita né frecce sposta. Nelle card, ordine editor: campi base → categorie/ingredienti → toggle **Menù personalizzabile** → prezzo; se il toggle e attivo il prezzo e svuotato/disabilitato. Card e carosello hanno blocchi prezzo separati, allineati a sinistra nel form, con simbolo `€` dentro il campo e senza testo descrittivo sotto. **Salva** (`commitSubTabEditor`) chiude il pannello.
...
- I dati visuali **Card scorrevole** sono sulla sottotab: `label`, `description`, `price_per_person`, `is_fixed_menu`, `hidden_*`, `preset_id`.
- Il **Carosello** salva testi/icona per slide in `carousel_items[]` (`eyebrow`, `title`, `description`, `icon`); il titolo tecnico nell'editor non e modificabile e `sub_tabs[].label` non viene sovrascritto dai testi slide né dal normalizer; può salvare `price_per_person`, ma non `is_fixed_menu` né `description` a livello sottotab carosello.
...
**Nessun prezzo** su slide. Il prezzo fisso del carosello, se presente e `is_fixed_menu !== false`, sta su `sub_tabs[].price_per_person` e viene mostrato nel riepilogo Prenota.
