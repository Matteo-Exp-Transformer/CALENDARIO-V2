...
| **Pagina Prenota v2 / BookingRequestPage / BookingRequestForm / card tipologia / sidebar riepilogo / BookingModeCards / BookingSubTabCards / BookingFormConfigPanel / booking_public_form_config / sub_tabs** | `docs/APP_CONTEXT_SKILL.md` §4 RULE Pagina Prenota v2 + `docs/per-ui-design-skill/UI_RESPONSIVE_SKILL.md` + `docs/per-ui-design-skill/UI_EDIT_SKILL.md` |
...
RULE  **Pagina Prenota v2** (`/prenota/:slug`): layout 2 colonne `lg:grid-cols-[1fr_min(360px,32%)]` (collassa a colonna unica sotto `lg`); ordine form come pagina attuale: prima sezione «Dati e Dettagli» (`BookingFormFields`: data, ora, ospiti, nome, contatti), poi «Tipo di prenotazione» con 3 card (`BookingModeCards`); se `sub_tabs_enabled` e `sub_tabs[]` non vuoto → strip card orizzontali scrollabili (`BookingSubTabCards`) sotto le tipologie; sidebar riepilogo (`BookingSummarySidebar`) a destra — sotto `lg` va dopo il form (`order-2`). Configurazione via `booking_public_form_config` in `restaurant_settings` (leggibile da anon grazie a policy `anon_select_restaurant_settings`); default in `bookingPublicFormConfig.ts`; parse/migrazione in `restaurantSettingRegistry.ts` (`sub_tabs[]` tipizzato; se vuoto ma presente `sub_tabs_overrides` legacy → migrazione runtime a `sub_tabs` tipo `preset`). Struttura sottotab: `SubTab` (`type: preset|manual`, `label`, `icon?`, `preset_id?`, `price_per_person?`, `description?`). Sottotab **preset** → card categorie orizzontali (`BookingMenuComposeGrid` + `BookingMenuCategoryCard` dentro `MenuSelection`; dropdown nativo nascosto se ci sono sottotab); sottotab **manuale** → niente griglia (`hideMenuGrid`), riga in sidebar, prefisso in `special_requests` al submit (`[Label - €X/p]`). Menù preselezionato staff (`booking_custom_staff_presets`, campo `is_fixed_menu`): se **fisso** → card mostrano solo `item_ids` e selezione read-only; se **personalizzabile** (`is_fixed_menu: false`) → tutti gli ingredienti bookable + header «CREA IL TUO MENU». Foto categoria da `menu_categories.image_url`. Filtri in `menuComposeVisibility.ts`. Sidebar sezione «Il tuo menu» con prefisso categoria. Il `booking_type` al submit resta quello della card padre. Admin: `BookingFormConfigPanel` + `MenuPricesTab` (toggle fisso). Componenti pubblici in `src/features/booking/components/publicBooking/`. Submit/payload/RPC invariati — non toccare `useCreateBookingRequest`.
```
...
| `BookingRequestPage.tsx` / `BookingRequestForm.tsx` / `BookingModeCards.tsx` / `BookingSubTabCards.tsx` / `BookingSummarySidebar.tsx` / `BookingFormFields.tsx` / `BookingSubTabStrip.tsx` (legacy, non usato nel form pubblico) | `APP_CONTEXT_SKILL.md` §4 RULE Pagina Prenota v2 |
| `bookingPublicFormConfig.ts` / `BookingFormConfigPanel.tsx` / `restaurantSettingRegistry.ts` chiave `booking_public_form_config` | `APP_CONTEXT_SKILL.md` §4 RULE Pagina Prenota v2 |
