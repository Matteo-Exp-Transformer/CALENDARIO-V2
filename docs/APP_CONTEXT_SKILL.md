...
RULE  **Pagina Prenota v2** (`/prenota/:slug`): layout 2 colonne `lg:grid-cols-[1fr_min(360px,32%)]` (collassa sotto `lg`); **ordine form (v2 attuale):** prima tipologia (`BookingModeCards`, 3 colonne compatte su mobile, descrizione solo da `sm+`), poi sottotab scrollabili (`BookingSubTabCards`, frecce desktop + touch), poi menù (`MenuSelection` → `BookingMenuComposeGrid`; mobile: colonna centrata al 75% con card collapsible `BookingMenuCategoryCard` layout `stack`; desktop: griglia o carosello), poi dati cliente (`BookingFormFields` + `DietaryRestrictionsSection` con `BookingPublicInsetField`: label **dentro** la card in alto a sinistra, valore sotto; larghezza `BOOKING_PUBLIC_CONTENT_WIDTH` = `mx-auto w-3/4` in `bookingPublicFieldStyles.ts`; **nessun** banner testo «Menù fisso» — solo UI read-only nelle card se `is_fixed_menu`). Sidebar `BookingSummarySidebar` a destra (`order-2` sotto `lg`). Config: `booking_public_form_config` in `restaurant_settings`; default `bookingPublicFormConfig.ts`; parse `restaurantSettingRegistry.ts`. Sottotab `SubTab` (`preset|manual`). Preset → griglia compose; manuale → `hideMenuGrid`, nota in `special_requests`. Menù staff: fisso = read-only card; personalizzabile = «CREA IL TUO MENU». Admin: `BookingFormConfigPanel` + `MenuPricesTab`. Pubblici in `publicBooking/`. Submit invariato — non toccare `useCreateBookingRequest`. Report: `docs/Sessioni di lavoro/25-05-26/Report-prenota-v2-ui-sessione-25-05-26.md`.
```
