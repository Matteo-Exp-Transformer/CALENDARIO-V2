...
Nota: **Asset preset sfondo (28-05-26).** Striscia laterale: 3 preset `strip-01..03` mappati a `public/asset/strip/strip-NN.png`. Pagina intera: 3 preset `full-01..03` mappati a `public/asset/sfondo intero/full-NN.png`. Naming file semplificato (più nomi con spazi/parentesi). Valori legacy `strip-04..06` / `full-04..06` salvati in DB: il parser li degrada in modo sicuro (`null` per strip, `DEFAULT_BOOKING_PAGE_BACKGROUND` per full).
