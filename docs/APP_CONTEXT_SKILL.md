...
| **Pagina Prenota v2 / BookingRequestPage / BookingRequestForm / card tipologia / sidebar riepilogo / BookingModeCards / BookingSubTabCards / booking_public_form_config / sub_tabs / layout striscia / sfondo / caselle form** | **`docs/per-ui-design-skill/BOOKING_REQUEST_PAGE_LAYOUT_CONTEXT.md`** + `UI_RESPONSIVE_SKILL.md` + `UI_EDIT_SKILL.md`. Modifiche a **Personalizza form** (`BookingFormConfigPanel`): leggere `BOOKING_FORM_CONFIG_PANEL_CURSOR_CONTEXT.md`. |
| **Tab Menu admin / MenuPricesTab / form ingrediente / categorie menu / promo testuali / menù preselezionati (preset staff) / booking_menu_promos / booking_custom_staff_presets** | **`docs/per-ui-design-skill/MENU_ADMIN_CONTEXT.md`** (+ `BOOKING_DATA_FLOW_SKILL.md` se tocca il flusso dati, `DB_SKILL.md` se tocca lo schema) |
...
RULE  **Servizio** (fasce/override a tempo, modal CRUD fascia, assegnazione tavoli, libera tavolo, quick assign da Calendario): **dettaglio completo → `docs/Dashboard-laterale-skill/ADMIN_PAGES_CONTEXT.md` § Servizio** (sottosezioni Fasce orarie · Assegnazione tavoli · Accesso rapido da Calendario).
RULE  Menu Prenota (`MenuPricesTab` = magazzino unico): categorie, form prodotto, promo testuali (`booking_menu_promos`), menù preselezionati (`booking_custom_staff_presets`). Foto categoria Prenota (`menu_categories.image_url`) ≠ foto thumbnail QR (`menu_homepage_config.category_images`) — non mischiare. Nessun omaggio automatico. **Dettaglio completo → `docs/per-ui-design-skill/MENU_ADMIN_CONTEXT.md`**; flusso dati/resolver → `docs/BOOKING_DATA_FLOW_SKILL.md`.
...
RULE  Admin **Personalizza form** (`BookingFormConfigPanel`) + tab **Anagrafica Azienda**: UI salvataggio condivisa `SettingsSaveUi.tsx`, autosave whitelist, guard navigazione, XOR card/carosello (`sub_tabs_presentation`), editor sottotab/carosello, card Sfondo (striscia vs pagina intera). **Dettaglio completo → `docs/per-ui-design-skill/BOOKING_FORM_CONFIG_PANEL_CURSOR_CONTEXT.md`**.
RULE  **Tracking personalizzazioni card Prenota** (`SubTab.field_overrides`): bandierine booleane (label/description/price/hidden_*) — `true` = personalizzato dal ristoratore (resta se preset cambia); `false`/assente = ereditato dal preset live. Resolver puro `bookingFormResolver.ts`. **Dettaglio completo → `docs/BOOKING_DATA_FLOW_SKILL.md`**.
RULE  **Pagina Prenota v2** (`/prenota/:slug`): layout esterno opzionale a 2 colonne (striscia foto `25vw` / contenuto) da 900px; sfondo viewport (striscia crema vs full-page); ordine form (tipologia → sottotab → menù → dati cliente); breakpoint **1256px** per sidebar riepilogo e sticky bar; XOR card/carosello; caselle single-row; validazione. Config in `booking_public_form_config`. Submit invariato — **non toccare `useCreateBookingRequest`**. **Dettaglio completo → `docs/per-ui-design-skill/BOOKING_REQUEST_PAGE_LAYOUT_CONTEXT.md`** (include il LOCK struttura griglia + tutte le note layout/sfondo/header/caselle/card ingredienti).
...
| `MenuPricesTab.tsx` / `MenuSelection.tsx` / `menuPricesCatalogLayout.ts` / `presetMenus.ts` / `menuCatalogGrouping.ts` | `MENU_ADMIN_CONTEXT.md` (+ `BOOKING_DATA_FLOW_SKILL.md`) |
...
| `BookingRequestPage.tsx` / `BookingRequestForm.tsx` / `BookingModeCards.tsx` / `BookingSubTabCards.tsx` / `BookingSummarySidebar.tsx` / `BookingStickyBar.tsx` / `BookingFormFields.tsx` / `BookingPublicInsetField.tsx` / `bookingPublicFieldStyles.ts` / `BookingSubTabStrip.tsx` (legacy) | `BOOKING_REQUEST_PAGE_LAYOUT_CONTEXT.md` (layout, LOCK griglia, note) |
| `bookingPublicFormConfig.ts` / `BookingFormConfigPanel.tsx` / `SettingsSaveUi.tsx` / `useDebouncedSettingsAutosave.ts` / `settingsAutosave.ts` / `restaurantSettingRegistry.ts` chiave `booking_public_form_config` | `BOOKING_FORM_CONFIG_PANEL_CURSOR_CONTEXT.md` |
| `MenuPricesTab.tsx` / `booking_menu_promos` / `booking_custom_staff_presets` / form ingrediente / categorie menu | `MENU_ADMIN_CONTEXT.md` (+ `BOOKING_DATA_FLOW_SKILL.md` per il flusso) |
| `MenuSelection.tsx` / `BookingMenuComposeGrid.tsx` / `BookingMenuCategoryCard.tsx` / `menuComposeVisibility.ts` / prop `hideMenuGrid` / `subTabOverrides` / `BookingMode.sub_tabs` | `BOOKING_REQUEST_PAGE_LAYOUT_CONTEXT.md` + `MENU_ADMIN_CONTEXT.md` |
