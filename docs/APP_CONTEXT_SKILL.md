...
Nota: **Griglia Data/Ora/Ospiti in `BookingFormFields` (28-05-26).** Breakpoint `sm` (≥640px): `grid-cols-[minmax(0,1fr)_9rem_7rem]` — Data flessibile, Ora 9rem, Ospiti 7rem. Precedentemente `[1fr_7rem_6rem]` rendeva la card Data dominante. Label "Data prenotazione \*" → **"Data \*"**. Mobile (<640px) invariato: colonna singola. Non tornare a `1fr` fisso per la colonna Data.
