...
Nota: per `display='carousel'` il resolver mantiene `price_per_person` salvato sulla sottotab; in Prenota `resolveCarouselSummaryDisplay` / `getCarouselStickyMiniPanelLine` in `bookingPublicFormConfig.ts` governano riepilogo laterale (`BookingSummarySidebar`) e mini-pannello sticky: toggle dettaglio ON + titoli slide → «Offerta selezionata» + lista; altrimenti solo prezzo se `price_per_person > 0` (anche con toggle ON ma senza titoli); toggle OFF senza prezzo → nessuna sezione offerta. Campo `show_offer_details_in_summary` su `sub_tabs[]`; editor carosello in `BookingFormConfigPanel` (blocco prezzo separato + toggle «Mostra dettaglio offerta»). Il carosello pubblico mantiene swipe/scroll mobile e mostra frecce laterali da desktop/tablet quando ci sono almeno 2 foto.
