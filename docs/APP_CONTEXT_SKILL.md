...
Nota: **Caselle compilazione single-row (28-05-26).** `BOOKING_PUBLIC_FIELD_BOX` ora è `flex-row items-center` (era `flex-col`): label a sinistra (shrink-0, nowrap) + valore/input a destra (`text-right`, `flex-1`) sulla stessa riga. Altezza compatta uniforme: `min-h-[2.5rem]` mobile / `sm:min-h-[2.75rem]` (~40-44px). Vale per tutte le caselle pubbliche Prenota: nome cliente, email, telefono, intolleranze, Data, Ora, ospiti. `BookingPublicInsetField`, `BookingPublicInsetFieldShell`, `BookingPublicDatePickerField`, `BookingPublicTimePickerField` condividono lo stesso layout. Per i picker date/time il button trigger usa `justify-end text-right` per allineare il valore selezionato a destra dopo l'icona.
...
Nota: **Sticky bar mobile** (`BookingStickyBar`): solo sotto 900px, fixed bottom, z-200. Appare quando `BookingSummarySidebar` esce dalla viewport (rilevato via `IntersectionObserver` + prop `onVisibilityChange`). Mostra mini-panel con titolo «Riepilogo Prenotazione», valori chiave (tipologia, data, ora, ospiti, totale) e pulsante submit. Clic sul mini-panel apre overlay bottom-sheet (z-300, max-h-90vh, flex-col): header cliccabile per chiudere (pill handle + titolo centrato + ChevronDown), contenuto scrollabile flex-1, footer submit fisso shrink-0. Tutti i colori usano CSS custom properties (`--color-warm-wood`, `--color-warm-orange`, `--color-terracotta`) via `color-mix` — si adattano automaticamente al tema attivo. `BookingRequestForm` espone prop `onIsDisabledChange` per sincronizzare lo stato disabled del submit verso il parent (`BookingRequestPage`). Il pulsante in overlay e nella barra usa `type="submit" form="booking-request-form"` (HTML nativo, nessun prop-drilling del handler). **Submit split per breakpoint**: `BookingSummarySidebar` mostra `submitButton` (slot ReactNode) solo su mobile/tablet sotto 900px (`block min-[900px]:hidden`); il pulsante grande «Invia Prenotazione» in `BookingRequestForm` è `hidden min-[900px]:flex` — visibile solo su desktop ≥900px. Il sidebar riepilogo è laterale fisso solo da 900px; su tablet (768-899px) e mobile appare in colonna sotto il form (la griglia del form usa `grid-cols-1` fino a `min-[900px]`, mai `md:grid-cols-*`). **Gap pulsante/sidebar → footer**: lo spazio prima del footer è gestito da uno spacer fisso `<div className="h-28 min-[900px]:h-6" aria-hidden />` come ultimo elemento della colonna destra in `BookingRequestPage`, prima della chiusura del tag colonna. `h-28` (~112px) riserva spazio per la sticky bar su mobile/tablet; `h-6` (~24px) su desktop. Il footer è fuori dalla griglia (`flex-col` > griglia > colonna | footer) — NON dentro la colonna destra. **Bug noti (28-05-26, da fixare)**: (A) desktop con sidebar corto: pulsante grande visivamente "troppo in alto" perché il form fields è più alto del sidebar nella griglia interna del form — soluzione possibile: `min-h` sul sidebar per allinearlo al form fields; (B) mobile/tablet: gap visivo sidebar→footer ancora percepito come ampio — valutare riduzione spacer da `h-28` a `h-20`. **Causa radice**: `BookingPhotoStrip` ha `sticky top-0 h-screen` — forza la griglia esterna ad avere altezza ≥100vh indipendentemente dal contenuto del form. Il footer si posiziona dopo la griglia, non dopo il contenuto. Lo spacer fisso nella colonna risolve il gap invariante ma non accorcia la pagina quando il form è corto. Vedi `docs/Sessioni di lavoro/28-05-26/Report-prenota-layout-gap-sessione-28-05-26-B.md`.
...
| `BookingRequestPage.tsx` / `BookingRequestForm.tsx` / `BookingModeCards.tsx` / `BookingSubTabCards.tsx` / `BookingSummarySidebar.tsx` / `BookingStickyBar.tsx` / `BookingFormFields.tsx` / `BookingPublicInsetField.tsx` / `bookingPublicFieldStyles.ts` / `BookingSubTabStrip.tsx` (legacy) | `APP_CONTEXT_SKILL.md` §4 RULE Pagina Prenota v2 |
