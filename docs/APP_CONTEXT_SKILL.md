...
Nota: **Bug stacking context foto full-page (28-05-26).** Il root `BookingRequestPage` non deve avere `position: relative` né `z-index` quando le foto full-page sono applicate via `<div fixed inset-0>`: il `relative` crea uno stacking context che pone i div figli con `z-index` negativo DIETRO al background del root stesso → risultato visivo "tutto scuro" (si vede solo il fallback). La regola è: root con solo `min-h-screen` + colore di fondo crema chiaro come fallback; div foto montati prima del contenuto, `pointer-events-none fixed inset-0` senza `-z-10`, naturalmente sotto nel flusso di rendering.
